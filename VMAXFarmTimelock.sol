...
/**
 * @dev VMAXFarm functions that do not require less than the min timelock
 */
interface IVMAXFarm {
    function add(
        uint256 _allocPoint,
...
contract TimelockController is AccessControl, ReentrancyGuard {
    ...
    function add(
        ...
        IVMAXFarm(_vmaxfarmAddress).add(0, _want, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
